...
Consumes per-platform digest artifacts (produced by `docker-build-digest`), pushes a multi-arch manifest list to the registry, then signs the image with cosign (keyless via OIDC) and attaches a SPDX-JSON SBOM as a signed attestation. Tags are generated by `docker/metadata-action` with `type=semver,pattern={{version}}`, and `latest` is added automatically by the action's `flavor: latest=auto` default.
